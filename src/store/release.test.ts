...
describe('the committed vdocs data-v1 record', () => {
  it('parses and pins the bundle + producer-manifest assets', () => {
    const record = loadReleaseRecord(
      new URL('../../contracts/releases/vdocs-data-v1.json', import.meta.url).pathname,
    );
    assert.equal(record.repo, 'rafael5/vdocs');
    assert.equal(record.tag, 'data-v1');
    for (const name of ['vdocs-data-v1.tar.gz', 'vdocs-data-v1.manifest.json']) {
