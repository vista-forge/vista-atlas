...
| Twin-link handlers (Atlas side of contract v1) | **done 2026-07-06 (openEntity excepted)** | `openSection`/`openDoc`/`search`/`pins` commands + the vscode:// URI handler registered; payloads now land the SPA on the target via the deep-link query (`?doc`/`?section`/`?q`+axes — SPA URL state added 2026-07-06, extension resolves section→doc and reloads the iframe; in-host smoke asserts the framed URL). `openEntity` awaits entity queries (P4) |
